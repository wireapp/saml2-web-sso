...
toServantErr (BadSamlResponseBase64Error msg) =
  err400 { errBody = "Bad response: base64 error: " <> cs msg }
toServantErr (BadSamlResponseXmlError msg) =
  err400 { errBody = "Bad response: xml parse error: " <> cs msg }
toServantErr (BadSamlResponseSamlError msg) =
  err400 { errBody = "Bad response: saml parse error: " <> cs msg }
toServantErr BadSamlResponseFormFieldMissing = err400
  { errBody = "Bad response: SAMLResponse form field missing from HTTP body"
  }
toServantErr BadSamlResponseIssuerMissing =
  err400 { errBody = "Bad response: no Issuer in AuthnResponse" }
toServantErr BadSamlResponseNoAssertions =
  err400 { errBody = "Bad response: no assertions in AuthnResponse" }
toServantErr BadSamlResponseAssertionWithoutID =
  err400 { errBody = "Bad response: assertion without ID" }
toServantErr (BadSamlResponseInvalidSignature msg) =
  err400 { errBody = cs msg }
toServantErr (InvalidCert msg) =
  err400 { errBody = "Invalid certificate: " <> cs msg }
toServantErr (BadServerConfig msg) =
  err400 { errBody = "Invalid server config: " <> cs msg }
toServantErr UnknownError =

...
hxtToConduit =
  either (throwError . ("hxtToConduit: parseLBS failed: " <>) . show) pure
    . parseLBS def
    . docToXML'

conduitToHxt :: MonadError String m => Document -> m HXT.XmlTree
conduitToHxt =
  either (throwError . ("conduitToHxt: xmlToDoc' failed: " <>)) pure
    . xmlToDoc'
    . renderLBS def { rsXMLDeclaration = False }

samlToConduit :: (MonadError String m, HXT.XmlPickler a) => a -> m Document
samlToConduit =
  either (throwError . ("samlToConduit: parseLBS failed: " <>) . show) pure
    . parseLBS def
    . HS.samlToXML
...
xmlToDoc' xml = case HXT.runLA HXT.xread (cs xml) of
  ...
  bad@(_ : _ : _) ->
    throwError $ "more than one root element: " <> show (length bad)
...
stripWhitespace = Uniplate.transformBis
  ...
  , [ Uniplate.transformer $ \case
        (Element nm attrs nodes) ->
          Element nm attrs (Prelude.filter (/= NodeContent "") $ nodes)
    ]
  ]
...
repairNamespacesEl el =
  unwrap . parseText def . renderText def . mkDocument $ el
 where

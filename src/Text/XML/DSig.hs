...
verifySelfSignature
  :: (HasCallStack, MonadError String m) => X509.SignedCertificate -> m ()
verifySelfSignature cert = do
  certToCreds cert >>= \case
    SignCreds SignDigestSha256 (SignKeyRSA pubkey) -> do
      ...
      unless
          (RSA.verify (Just Crypto.SHA256) pubkey signedMessage signatureValue)
...
parseKeyInfo
  :: (HasCallStack, MonadError String m) => LT -> m X509.SignedCertificate
parseKeyInfo (cs @LT @LBS -> lbs) =
  case HS.xmlToSAML @HS.KeyInfo =<< stripWhitespaceLBS lbs of
    Right keyinf -> case HS.keyInfoElements keyinf of
      HS.X509Data (HS.X509Certificate cert :| []) :| [] -> {- verifySelfSignature cert >> -}pure cert
      HS.X509Data (HS.X509Certificate _ :| bad) :| bad' ->
        throwError $ "unreadable trailing data or noise: " <> show (bad, bad')
      unsupported ->
        throwError
          $  "expected exactly one KeyInfo element: "
          <> show unsupported
    Left errmsg ->
...
stripWhitespaceLBS lbs =
  renderLBS def . stripWhitespace <$> fmapL show (parseLBS def lbs)
...
certToPublicKey
  ...
  => X509.SignedCertificate
  -> m RSA.PublicKey
certToPublicKey cert =
  certToCreds cert <&> \(SignCreds _ (SignKeyRSA key)) -> key
...
mkSignCreds
  :: (Crypto.MonadRandom m, MonadIO m) => Int -> m (SignPrivCreds, SignCreds)
mkSignCreds size =
  mkSignCredsWithCert Nothing size <&> \(priv, pub, _) -> (priv, pub)
...
mkSignCredsWithCert mValidSince size = do
  ...
  validSince :: Hourglass.DateTime <- maybe (liftIO Hourglass.dateCurrent)
                                            pure
                                            mValidSince
  let validUntil = validSince
        `Hourglass.timeAdd` mempty { Hourglass.durationHours = 24 * 365 * 20 }
  ...
  cert <- X509.objectToSignedExactF
    signcert
    X509.Certificate
      { X509.certVersion      = 2 :: Int
      , X509.certSerial       = 387928798798718181888591698169861 :: Integer
      , X509.certSignatureAlg = X509.SignatureALG X509.HashSHA256
                                                  X509.PubKeyALG_RSA
...
verifyRoot
  :: forall m . (MonadError String m) => NonEmpty SignCreds -> LBS -> m ()
verifyRoot creds el = do
  signedID <- do
    XML.Document _ (XML.Element _ attrs _) _ <- either
      (throwError . ("Could not parse signed document: " <>) . cs . show)
      pure
      (XML.parseLBS XML.def el)
    maybe
      (  throwError
      $  "Could not parse signed document: no ID attribute in root element."
      <> show el
      )
      (pure . cs)
      (Map.lookup "ID" attrs)
...
verifyIO
  ...
  -> LBS
  -> String
  -> IO [(SignCreds, Either HS.SignatureError ())]
...
verifyIO' (SignCreds SignDigestSha256 (SignKeyRSA key)) el signedID =
  runExceptT $ do
...
signRoot
  :: (Crypto.MonadRandom m, MonadError String m)
  => SignPrivCreds
  -> XML.Document
  -> m XML.Document
...
signRootAt
  ...
  -> XML.Document
  -> m XML.Document
signRootAt sigPos (SignPrivCreds hashAlg (SignPrivKeyRSA keypair)) doc = do
  (docWithID :: XML.Document, reference) <- addRootIDIfMissing doc
  docInHXT                               <- conduitToHxt docWithID

  let
    canoAlg = HS.CanonicalXMLExcl10 True
    transforms =
      Just
        .  HS.Transforms
        $  HS.Transform
             { HS.transformAlgorithm           = HS.Identified
                                                   HS.TransformEnvelopedSignature
             , HS.transformInclusiveNamespaces = Nothing
             , HS.transform                    = []
             }
        :| [ HS.Transform
               { HS.transformAlgorithm           =
                 HS.Identified (HS.TransformCanonicalization canoAlg)
               , HS.transformInclusiveNamespaces = Nothing
               , HS.transform                    = []
               }
           ]
  ...
  docCanonic :: SBS <-
    either (throwError . show @SomeException) (pure . cs)
    . unsafePerformIO
    . try
  ...
  let digest :: SBS
      digest = case hashAlg of
        SignDigestSha256 ->
          ByteArray.convert $ Crypto.hash @SBS @Crypto.SHA256 docCanonic

  let
    signedInfo = HS.SignedInfo
      { signedInfoId                     = Nothing :: Maybe HS.ID
      , signedInfoCanonicalizationMethod = HS.CanonicalizationMethod
                                             (HS.Identified canoAlg)
                                             Nothing
                                             []
      , signedInfoSignatureMethod        = HS.SignatureMethod
                                             (HS.Identified HS.SignatureRSA_SHA256)
                                             Nothing
                                             []
      , signedInfoReference              =
        HS.Reference
            ...
            , referenceDigestMethod = HS.DigestMethod
                                        (HS.Identified HS.DigestSHA256)
                                        []
...
injectSignedInfoAtRoot
  :: MonadError String m
  => Int
  -> HS.Signature
  -> XML.Document
  -> m XML.Document
injectSignedInfoAtRoot sigPos signedInfo (XML.Document prol (Element tag attrs nodes) epil)
  = do
    when (sigPos > Prelude.length nodes) $ do
      throwError
        (  "child list too short: is "
        <> show (Prelude.length nodes)
        <> ", need "
        <> show sigPos
        )
    ...
    pure $ XML.Document
      prol
      (Element tag attrs (insertAt sigPos (XML.NodeElement signedInfoXML) nodes)
      )
      epil
 where
  insertAt :: Int -> a -> [a] -> [a]
  insertAt pos el els = case Prelude.splitAt pos els of
    (prefix, suffix) -> prefix <> [el] <> suffix
...
signElementIO
  :: (HasCallStack, HasMonadSign m) => SignPrivCreds -> [Node] -> m [Node]

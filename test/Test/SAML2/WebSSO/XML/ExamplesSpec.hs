...
spec = describe "XML serialization" $ do
  describe "unit tests" $ do
    it "Time seconds have no more than 7 decimal digits" $ do
      ...
      renderTime (unsafeReadTime "2013-03-18T03:28:54.1839884Z")
        `shouldBe` renderTime
                     (unsafeReadTime "2013-03-18T03:28:54.18398841817Z")
      ...
      forM_ decimalses $ \decimals -> do
        ...
        renderTime (unsafeReadTime good)
          `shouldBe` renderTime (unsafeReadTime bad)

    roundtrip 0
              (readSampleIO "microsoft-authnrequest-1.xml")
              Samples.microsoft_authnrequest_1
  ...
  describe "AuthnRequest" $ do
    it "works" $ do
      ...
      decodeElem @AuthnRequest @(Either String) (encodeElem req)
        `shouldBe` Right req
  ...
  describe "AuthnResponse with tricky Subject elements" $ do
    ...
    let
      check :: HasCallStack => String -> [Node] -> Maybe ST -> Spec
      check msg nameId expectedsubj = it msg $ do
        ctx :: CtxV              <- mkTestCtxSimple
        spmeta :: SPMetadata     <- ioFromTestSP ctx mkTestSPMetadata
        authnreq :: AuthnRequest <- ioFromTestSP ctx
          $ createAuthnRequest 3600 defSPIssuer
        SignedAuthnResponse doc <- ioFromTestSP ctx $ mkAuthnResponseWithRawSubj
          nameId
          sampleIdPPrivkey
          testIdPConfig
          spmeta
          authnreq
          True
        let parsed :: Either String AuthnResponse =
              parseFromDocument @AuthnResponse doc
        case expectedsubj of
          Nothing -> do
            parsed `shouldSatisfy` isLeft
          Just subj -> do
            parsed `shouldSatisfy` isRight
            let Right (subjid :: NameID) =
                  parsed
                    <&> (^. assertionL . assContents . sasSubject . subjectID)
            shortShowNameID subjid `shouldBe` Just subj

      mknid =
        NodeElement
          . Element "{urn:oasis:names:tc:SAML:2.0:assertion}NameID" mempty

    check "good" [mknid [NodeContent "xkcd"]] (Just "xkcd")

    check "NameID with fragmented contents"
          [mknid [NodeContent "we", NodeContent "f"]]
          (Just "wef")

    check "no NameID"    []             Nothing

    check "empty NameID" [mknid mempty] Nothing

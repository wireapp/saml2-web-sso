...
tests =
  [ ("instance HasXML Document"    , mkpropHasXML' normalizeDoc genXMLDocument)
  , ("instance HasXML NameID"      , mkpropHasXML genNameID)
  , ("instance HasXML IdPMetadata" , mkpropHasXML genIdPMetadata)
  , ("instance HasXML SPMetadata"  , mkpropHasXML genSPMetadata)
  , ("instance HasXML AuthnRequest", mkpropHasXML genAuthnRequest)
  , ( "instance HasXML Conditions"
    , mkpropHasXML' canonicalizeConditions . Gen.prune $ genConditions
    )
  , ( "instance HasXMLImport Conditions"
    , mkpropHasXMLImport' canonicalizeConditions (Proxy @HS.Conditions)
      . Gen.prune
      $ genConditions
    )
  ...
  , ("instance {From,To}JSON Config", mkpropJSON genConfig)
  , ("instance {From,To}JSON IdPId" , mkpropJSON genIdPId)
  , ("instance {From,To}JSON Issuer", mkpropJSON genIssuer)
  , ("instance {From,To}JSON URI"   , mkpropJSON genHttps)
  , ( "instance {From,To}JSON SignedCertificate"
    , mkpropJSON genSignedCertificate
    )
  , ("instance FromHttpApiData IdPId", mkpropHttpApiData genIdPId)
...
canonicalizeAuthnResponse =
  rspPayload %~ fmap (assConditions . _Just %~ canonicalizeConditions)


mkpropHasXML :: forall a . (Eq a, Show a, HasXML a) => Gen a -> Property
mkpropHasXML = mkpropHasXML' id

mkpropHasXML'
  :: forall a . (Eq a, Show a, HasXML a) => (a -> a) -> Gen a -> Property
...
mkpropMimeRender
  ...
  -> Gen a
  -> Property
mkpropMimeRender proxy gen = scaledprop $ Hedgehog.forAll gen >>= \a ->
  Hedgehog.tripping a (mimeRender proxy) (mimeUnrender proxy)

mkpropJSON
  :: forall a . (Eq a, Show a, FromJSON a, ToJSON a) => Gen a -> Property
mkpropJSON gen = scaledprop $ Hedgehog.forAll gen >>= \a ->
  Hedgehog.tripping a Aeson.encode Aeson.decode
...
mkpropHttpApiData
  ...
  => Gen a
  -> Property
mkpropHttpApiData gen = scaledprop $ Hedgehog.forAll gen >>= \a ->
  Hedgehog.tripping a toUrlPiece parseUrlPiece

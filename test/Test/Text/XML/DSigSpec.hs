...
spec = describe "xml:dsig" $ do
  describe "parseKeyInfo" $ do
    it "works(1)" $ do
      ...
      let want = Samples.microsoft_idp_keyinfo
          Right (SignCreds _ (SignKeyRSA have)) =
            certToCreds =<< parseKeyInfo keyinfo
      have `shouldBe` want
  ...
  describe "verify" $ do
    it "works" $ do
      Right keyinfo <- (parseKeyInfo >=> certToCreds)
        <$> readSampleIO "microsoft-idp-keyinfo.xml"
      raw <- cs <$> readSampleIO "microsoft-authnresponse-2.xml"
      verify (keyinfo :| []) raw "_c79c3ec8-1c26-4752-9443-1f76eb7d5dd6"
        `shouldBe` Right ()
    ...
    it "works with more than one key" $ do
      Right keyinfo <- (parseKeyInfo >=> certToCreds)
        <$> readSampleIO "microsoft-idp-keyinfo.xml"
      raw <- cs <$> readSampleIO "microsoft-authnresponse-2.xml"
      verify (keyinfo :| [sampleIdPPubkey])
             raw
  ...
  describe "verifyRoot vs. signRoot" $ do
    let
      ...
      _mkcrdsReal = \case
        ...
        False ->
          (,)
            <$> (fst <$> mkSignCreds keysize)
            <*> (snd <$> mkSignCreds keysize)
      ...
      doc withID = Document (Prologue [] Nothing [])
                            (Element "root" (someID withID) root)
                            []
      root =
        [ NodeElement
            (Element
              "bloo"
              (Map.fromList [("hign", "___")])
              [ NodeElement
                (Element "ack"
                         (Map.fromList [("hoghn", "true")])
    ...
    it "keeps data intact" $ do
      ...
      Right outcome          <- runMonadSign
        (cs . renderLBS def <$> signRoot privCreds (doc False))
    ...
    it "honors non-0 signature position." $ do
      ...
      case signed of
        Document _ (Element "root" _ [NodeElement (Element "bloo" _ _), NodeElement (Element "{http://www.w3.org/2000/09/xmldsig#}Signature" _ _)]) _
          -> pure ()
        bad -> error $ show bad

    it "throws an error is signature position points outside the children list."
      $ do
          (privCreds, _pubCreds) <- mkcrds True
